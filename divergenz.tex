...
\begin{itemize}
  \item $S_1 = (\{q_{01}\},\emptyset ,\emptyset , \emptyset, q_{01},
    \{q_{01}\})$ (siehe auch Abbildung~\ref{S1}),
  \item $S_2 = (\{q_{02}\},\emptyset ,\emptyset , \{(q_{02},\tau ,q_{02})\},
    q_{02}, \emptyset )$ (siehe auch Abbildung~\ref{S2}),
  \item $U = (\{q_{0U}\}, \emptyset, \{\omega\}, \{(q_{0U},\omega ,q_{0U})\},
    q_{0U}, \emptyset )$ (siehe auch Abbildung~\ref{U}).
\end{itemize}
\begin{figure}[h!tbp]
...
\label{S1}
  ...
  \begin{center}
    \begin{tikzpicture}[->, >=latex',auto,node distance =3cm, semithick]
      \node (0) [rectangle, dashed, draw]{$q_{02}\in Div_2$};
...
\label{S2}
  ...
  \begin{center}
    \begin{tikzpicture}[->, >=latex',auto,node distance =3cm, semithick]
      ...
      \path ($ (0) + (-1,0) $) edge (0)
            (0) [loop right] edge node {$\omega !$} (0);
    \end{tikzpicture}
    \caption{$U$}
\label{U}
  \end{center}
...
Für die Parallelkompositionen $U\|S_1$ und $U\|S_2$ gilt dann:
\begin{itemize}
  \item $U\|S_1 =(\{(q_{0U},q_{01})\},\emptyset ,\{\omega\}, \{((q_{0U},
    q_{01}),\omega ,(q_{0U},q_{01}))\}, (q_{0U}, q_{01}),
    \{(q_{0U},q_{01})\})$ (siehe auch Abbildung~\ref{US1}),
  \item $U\|S_2 =(\{(q_{0U},q_{02})\},\emptyset ,\{\omega\}, \{((q_{0U},
    q_{02}),\omega ,(q_{0U},q_{02})), ((q_{0U},q_{02}),\tau
  ,(q_{0U},q_{02}))\}, (q_{0U},$ $q_{02}), \emptyset)$ (siehe auch
  Abbildung~\ref{US2}).
\end{itemize}
\begin{figure}[h!tbp]
  ...
  \begin{center}
    \begin{tikzpicture}[->, >=latex',auto,node distance =3cm, semithick]
      \node (0) [rectangle, draw]{$(q_{0U},q_{01})\in E_{U1}$};
...
\label{US1}
  ...
  \begin{center}
    \begin{tikzpicture}[->, >=latex',auto,node distance =3cm, semithick]
      \node (0) [rectangle, dashed, draw]{$(q_{0U},q_{02})\in Div_{U2}$};

      \path ($ (0) + (-2.5,0) $) edge (0)
            (0) [loop right] edge node {$\omega !,\tau$} (0);
    \end{tikzpicture}
    \caption{$U\|S_2$}
\label{US2}
  \end{center}
  \end{minipage}
\end{figure}
Somit stehen die beiden Transitionssysteme $U\|S_1$ und $U\|S_2$ in der
Basisrelation \DBRel{}, obwohl in $U\|S_1$ nur ein Error und in $U\|S_2$ nur
ein Divergenz-Zustand lokal erreichbar ist. Um ein analoges Lemma zu den
Lemmata~\ref{lemVerfeinerung} und~\ref{lemQuiVerfeinerung} formulieren zu
können sollten somit auch $S_1$ und $S_2$ in der Relation \DRel{} stehen. Es
kann also unter der Verfeinerungsrelation \DRel{} nicht unterschieden
werden, ob das System Error oder Divergenz als Fehler hat.
...
\begin{proof}
  ...
  präfix-minimales Element $w$ zu betrachten. Es muss hier unterschieden werden, ob
  ...
  ausführbar ist, automatisch ein Error in der Parallelkomposition entsteht.
...
\begin{proof}
  ...
  in Lemma~\ref{lemQuiVerfeinerung}, aus der lokalen Erreichbarkeit eines Errors in
...
\label{UohneEmitO}
      ...
      \begin{itemize}
        \item Fall 2i) ($\varepsilon\in\ET{}(U\|S_2)$ wegen neuem Error): Da
          ...
          dann $q_{02} \overset{x_1\dots x_i}{\Longrightarrow} q'
        ...
        \item Fall 2iii) ($\varepsilon\in \DT{}(U\|S_2)\backslash
          ...
          von $w$ ist und $\DT{}_2\subseteq \EDT{}_2$ gilt, ist in diesem Fall
          ein Präfix von $w$ in $\EDT{}_2$ enthalten.
      \end{itemize}
  \end{itemize}
  ...
  \begin{itemize}
    \item Fall 1 ($w=\varepsilon$): Es ist ein Ruhe-Zustand intern erreichbar
      ...
      oder Divergenz-Zustand lokal erreichbar. Für $U$ wird das folgende
      Transitionssystem verwendet (siehe auch Abbildung~\ref{UepsQDT}):
      \begin{itemize}
        \item $Q_U=\{q_0, q\}$,
        \item $q_{0U}=q_0$,
        \item $E_U=\emptyset$,
      ...
      \begin{center}
        \begin{tikzpicture}[->, >=latex',auto,node distance =3cm, semithick]
          ...
          \path ($ (0) + (-2,0) $) edge (0)
                ...
                (q) edge [loop right] node {$x?\in I_U, \omega !$} (q);
        \end{tikzpicture}
        \caption{$q_0$ ist der einzige Ruhe-Zustand}
\label{UepsQDT}
      \end{center}
      \end{figure}
      Es ist auch in $U$ ein Ruhe-Zustand intern erreichbar. Zusätzlich kann
      in der Komposition durch den Aufbau von $U$ kein neuer Error entstehen.
      Daraus folgt mit dem ersten Punkt von Lemma~\ref{lemRuheParallelkomp}, dass auch in $U\|S_1$
      ein Ruhe-Zustand lokal erreichbar ist. Es muss also auch in $U\|S_2$ ein
      ...
      dies ist jedoch aufgrund der Definition der Menge \QDT{} nicht relevant.
      Da in $U$ nur durch $\varepsilon$ ein Ruhe-Zustand erreichbar ist, kann nur $\varepsilon$ ein
      striktes Ruhetrace sein, falls es sich bei dem Fehler in $U\|S_2$ um
      einen lokal erreichbaren Ruhe-Zustand handelt. Somit folgt in allen drei
      Fällen, dass $w$ in $\QDT{}_2$ enthalten ist.
...
\label{UohneEmitIundO}
      ...
      \begin{itemize}
        \item Fall 2a) ($\varepsilon\in \ET{}(U\|S_2)$): Der lokal erreichbare
          Fehler ist ein Error. Das $w$ muss somit nicht ausführbar sein. Der
          ...
          Sychornisa\-tions-Fehler möglich sind und wegen den Fällen 2i) und
        ...
        \item Fall 2c) (Ruhe-Zustand lokal erreichbar in $U\|S_2$ und
          ...
          erreicht werden kann. Mit dem zweiten Punkt von Lemma~\ref{lemRuheParallelkomp} kann somit
          gefolgert werden, dass auch in $S_2$ ein Ruhe-Zustand mit $w$
          erreichbar sein muss, da $\ET{}(U\|S_2)\subseteq \EDT{}(U\|S_2)$ ist.
          Es gilt also $w\in \StQT{}_2\subseteq \QDT{}_2$.
      \end{itemize}
  \end{itemize}
...
unterscheiden könnte. Es würde dann die Relation \ERel{} erhalten bleiben
und man hätte die Inklusionen $\DT{}_1 \cup\ET{}_1 \subseteq \DT{}_2\cup
\ET{}_2$ und $\QT{}_1\cup \DT{}_1\subseteq \QT{}_2\cup \DT{}_2$ für die
Verfeinerungsrelation in diesem Kapitel. Dies würde dem Ansatz
aus~\cite{Chilton2013} entsprechen. Dadurch würden jedoch Error als
\glqq{}schlimmerer Fehler\grqq{} aufgefasst, wie Divergenz. Es gäbe also eine
echte Hierarchie zwischen den Fehler.
