...
Lösung möglich. Dies wird an folgendem Beispiel illustriert. Hier für werden
...
Für die Parallelkomposition $U\|S_1$ und $U\|S_2$ gilt dann:
\begin{itemize}
...
Basisrelation \DBRel{}. Um ein analoges Lemma zu den
...
kann also in unter der Verfeinerungsrelation \DRel{} nicht unterschieden
...
\begin{proof}
  ...
  Es wird ebenso wie oben nur ein präfix-minimales $x$ betrachtet wegen den
  ...
  jeweils kein weiterer Unterfall für $v_2'$ notwendig, da wenn es nicht
  ausführbar ist automatisch ein Error in der Parallelkomposition entsteht.
  Somit ist egal, ob auch noch Divergenz vorlag. Falls $v_2'$ ausführbar
  ist, ist nicht relevant, ob eine Divergenz-Möglichkeit bestanden hat, da diese
  ...
  lässt sich auch hier Lemma~\ref{lemRuheParallelkomp} anwenden.
...
\label{UepsQDT}
      \end{center}
      \end{figure}
      Es ist auch in $U$ ein Ruhe-Zustand lokal erreichbar. Zusätzlich kann
      in der Komposition durch den Aufbau von $U$ kein neuer Error entstehen.
      Daraus folgt mit Lemma~\ref{lemRuheParallelkomp}, dass auch in $U\|S_1$
...
\label{UohneEmitIundO}
      ...
      \begin{itemize}
        \item Fall 2a) ($\varepsilon\in \ET{}(U\|S_2)$): Der lokal erreichbare
          ...
          synchonizations Möglichkeiten als neuer Error in der
          ...
          2ii) des ersten Punktes von dieses Beweises, ist ein Präfix von $w$
...
Indem man eine Basisrelation definieren würde, die unterscheiden kann, ob es
...
unterscheiden könnte. Es würde dann also die Relation \ERel{} erhalten bleiben
...
aus~\cite{Chilton2013} entsprechen.
