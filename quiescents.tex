...
\begin{proof}
  %TODO umformulieren, damit sortierter, möglicherweise als Widerspruchsbeweis,
  %siehe bei 2. auch Anmerkung aus Korrektur
  1.:\\
...
\begin{proof}
  ...
  $U\|S_1$, wenn in $U$ kein Ruhe-Zustand durch $w$ erreichbar wäre.
  \begin{itemize}
    \item Fall 1 ($w=\varepsilon$): Es ist ein Ruhe-Zustand intern erreichbar in
      ...
      Transitionen möglich sind, kann nur $\varepsilon$ ein strikter
    ...
    \item Fall 2 ($w=x_1\dots x_n\in \Sigma ^+$ mit $n\geq 1$): Es wird der
      ...
      \begin{center}
        \begin{tikzpicture}[->, >=latex',auto,node distance =3cm, semithick]
          ...
          \node (1) [right of=0] {$q_1$};
          \node (dots) [right of=1] {$\dots$};
          \node (n) [right of=dots, rectangle, draw] {$q_n\in Qui_U$};
          \node (q) [below of=1] {$q$};
      ...
      erreicht werden. Da $w\notin \ET{}(S_1)$ gilt, kann auf dem Weg, der mit
      ...
      Error entsehen, da für $q_n$ keine Inputs möglich sind. Falls es jedoch
      durch Synchronisation zu einem Error kommt, ist dies nur mit einer
      Verlängerung des $w$s um einen Input aus $I_U$ möglich. Da jedoch egal
      wie, immer ein Ruhe-Zustand in $U\|S_2$ lokal erreichbar ist, muss auch ein
      ...
      \begin{itemize}
        \item Fall 2a) ($\varepsilon\in \ET{}(U\|S_2)$): Es handelt sich bei
          ...
          da $U$ nur Synchronisationsfehler auf dem Trace $w$ zulässt. Da
...
\begin{satz}[Ruhe-Präkongurenz bzgl. Internalisierung]
  ...
  $\cdot /\cdot$.
  %TODO Anpassen an entsprechenden Error-Satz
\end{satz}
...
% möglich ist. %TODO Umformulieren und dann für alternative Ruhe-Definition verwenden

\begin{Def}[Divergenz]
  Ein \emph{Divergenz-Zustand} ist ein Zustand in einem \EIO{}, der eine
  unendliche Folge an $\tau$s ausführen kann. %TODO in eigenes Kapitel, als
  %eigenen Verneinungsfall, formale Def möglicherweise mit unendlicher
  %indizierter Folge, da Q unendlich sein kann
\end{Def}
