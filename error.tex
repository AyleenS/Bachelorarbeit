...
gröbste Präkongruenz definiert, die gesucht werden soll. Somit kann überprüft
werden, ob eine andere gefundene Präkongruenz auch die Eigenschaft erfüllt die
gröbste zu sein.
...
Menge der Traces benötigt, die zu einem Zustand führen, von dem aus mit lokalen Aktionen
...
deutlich allgemeiner wie die Menge $\StET{}$. Zusätzlich wird auch noch
...
\begin{proof}
  ...
  gelten muss, kann nicht mehr für alle Fälle
  $q_{02}\overset{v_2'}{\Rightarrow}$ gefolgert werden, wie das in~\cite{Vogler2014EIO}
  möglich war.
  \begin{itemize}
    \item Fall I ($v_1\in \MIT{}_1$ und $v_1\neq\varepsilon$): Es gibt $q_{01}
      ...
      \begin{itemize}
        \item Fall Ia) ($a\notin\Sigma _2$): Es gilt $q_{02}
          ...
          und $v_2$ nicht auf $a$ enden können. Dadurch erhält man
        ...
        \item Fall Id) ($a\in O_2$): Es gilt $v_2'=v_2''a$ und $q_{02}
          ...
          dem $a$ entsprechen kann. Man erhält also $q_{02}
          \overset{v_2''}{\Rightarrow} q_2 \overset{a}{\rightarrow}$ mit
  ...
  \item Fall II ($v_1\in \PrET{}_1$): $\exists u_1\in O_1^*:q_{01}
    ...
    in~\cite{Vogler2014EIO} gibt, kann
...
dann die erste Eigenschaft erfüllt ist, um eine Präkongruenz zu finden, die
    ...
    \item Fall 2 ($w=x_1\dots x_n$ mit $n\geq 1$): Es wird einen
      ...
      \begin{itemize}
        ...
        \item Fall 2c) (geerbter Error von $U$): Da der einzige Zustand aus
          ...
          diesem Fall gilt, wie im oben, $w\in L(S_2)\subseteq \EL{}(S_2)$.
...
Der folgende Satz sagt aus, dass \ERel{} die gröbste Präkongruenz ist, die gesucht
wurde und somit äquivalent ist zur vollständig abstrakten
Präkongruenz \ECRel{}.
...
\begin{proof}
  ...
  \glqq $\Leftarrow$\grqq : Nach Definition gilt, wenn
      ...
      \ET{}(S_2)$ gilt, wenn $\varepsilon\in \ET{}(S_1)$. Dadurch folgt, dass
      $S_1\EBRel S_2$ gilt, da \EBRel{} in Definition~\ref{DefErrorBasisrel}
      über die lokale Erreichbarkeit der Error-Zustände definiert wurde. Somit
      ist $S_1\ERel S_2$ eine Präkongruenz, die in \EBRel{} enthalten ist. Da
      \ECRel{} die gröbste Präkongruenz bezüglich $\cdot\|\cdot$ ist, die in
      \EBRel{} enthalten ist, muss \ERel{} in \ECRel{} enthalten sein. Es folgt
      also aus $S_1\ERel S_2$, dass auch $S_1\ECRel{} S_2$ gilt.
...
\begin{proof}
  ...
  Error, der in $S_i$ lokal erreichbar ist über einen Trace, der einen
  Outputs aus $\{x_1,x_2,\dots ,x_n\}$ enthält, auch in $S_i/\{x_1,x_2,\dots
  ,x_n\}$ erreichbar, jedoch enthält der Trace nicht mehr diesen Output. Alle
  Traces, die keine Outputs aus der Menge hinter dem Internalisierungsoperator
...
\begin{satz}[Präkongurenz bzgl. Interalisierung]
  ...
  auch $(S_1/\{x_1,x_2,\dots ,x_n\})\ERel (S_2/\{x_1,x_2,\dots ,x_n\})$. Daraus
  folgt insbesondere, dass \ERel{} eine Präkongruenz bezüglich $\cdot /\cdot$
  ist.
\end{satz}

\begin{proof}
  Da $S_1\ERel S_2$ gilt, weiß man, dass $\ET{}_1\subseteq \ET{}_2$ und
  $\EL{}_1\subseteq \EL{}_2$ gilt. Es gilt $X:=\{x_1,x_2,\dots ,x_n\}\subseteq
  O$, da in der Definition des Internalisierungsoperators keine andere $x_i$
  zulässig sind. Eine genauere Vorstellung davon zu bekommen, was
  das Hiding für die Systeme bedeutet, werden hier zunächst einmal die
  neuen Errortraces und die neue error-geflutete Sprache definiert. Es gilt $\ET{}(S/X) =
  \cont{}(\PrET{}(S/X))\cup \cont{}(\MIT{}(S/X))$ mit $\StET{}(S/X) = \{w\in
  (\Sigma\backslash (X\cap O))^*\mid \exists w'\in \StET{}(S):
  w'|_{\Sigma\backslash (X\cap O)} = w\}$, $\PrET{}(S/X) = \{\prune (w)\mid
  w\in\StET{}(S/X)\}$ und $\MIT{}(S/X) = \{w\in (\Sigma\backslash (X\cap
  O))^*\mid \exists w'\in \MIT{}(S): w'|_{\Sigma\backslash (X\cap O)} = w\}$.
  Die error-geflutete Sprache \EL{} wird nun analog dazu definiert als
  $\EL{}(S/X) = L(S/X)\cup \ET{}(S/X)$ mit $L(S/X) = \{w\in (\Sigma\backslash
  (X\cap O))^*\mid \exists w'\in L(S): w'|_{\Sigma\backslash (X\cap O)} =
  w\}$.\\
  Es wird einen Trace $w=a_1a_2\dots a_m\in \ET{}_1$ gewählt. Solang $w$ nicht nur
  aus Outputs besteht, gibt es also einen Ablauf $q_0
  \overset{w'}{\Rightarrow}$ mit $w'=a_1a_2\dots a_k$, so dass $a_k\in I$,
  $k\leq m$ und $w'\in \MIT{}_1$ oder $w'\in \PrET{}_1$ gilt. Nach der
  Internalisierung bleiben von dem Ablauf nur noch die Aktionen übrig, die
  nicht Element von $X$ sind. Der Trace reduziert sich also auf
  $w''=w|_{\Sigma\backslash (O\cap X)}$. Diese $w''$ hat einen zu $w'$ analoges
  Präfix, da der Input $a_k$ erhalten bleibt. Dieses Präfix von $w''$ ist dann
  in $\MIT{}(S_1/X)$ oder in $\PrET{}(S_1/X)$ enthalten und somit ist $w''$ in
  $\ET{}(S_1/X)$ enthalten. Diese Argumentation funktioniert jedoch nicht für
  $w$s, die nur aus Outputs bestehen. Da Elemente aus $\cont{}(\MIT{}_1)$
  mindestens einen Input enthalten, muss so ein $w$ aus
  $\cont{}(\PrET{}_1)$ sein. Somit ergibt sich ohne die \cont{}-Funktion das
  Element $\varepsilon\in\PrET{}_1$. Da es in diesem Wort keine Elemente aus
  $X$ gibt, werden in diesem Zusammenhang auch keine Aktionen verborgen und somit gilt
  $\varepsilon\in\PrET{}(S_1/X)$.\\
  ...
  kann geschlossen werden, dass alle relevanten Traces bereits in $\EL{}_2$ enthalten sind
  und die Modifikation durch den Internalisierungsoperator wie oben keine
  Auswirkungen auf die Teilmengenbeziehung hat. %TODO überlegen ob die beliebe
  %Fortsetzung problematisch ist (nicht problematisch, sollte aber explizit mit
