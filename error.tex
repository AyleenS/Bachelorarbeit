  ...
  \begin{center}
    \begin{tikzpicture}
      \matrix (m) [matrix of math nodes,row sep=3em,column sep=4em,minimum width=2em]{
        ...
        \forall~\mathrm{komponierbare}~U:U\|S_1\EBRel U\|S_2 \\};
        \path[-stealth]
          (m-1-1) edge [double] node [above] {\glqq{}$\Leftarrow$\grqq{} von
            Satz~\ref{satzFullAbstractness}} (m-1-2)
          (m-1-2) edge [double] node [right] {\glqq{}$\Rightarrow$\grqq{} von
            Satz~\ref{satzFullAbstractness}} (m-2-2)
          (m-2-1) edge [double] node [left] {Lemma~\ref{lemVerfeinerung}} (m-1-1)
          (m-2-2) edge [double] node [below] {$U$ Partner $\Rightarrow U$ kombonierbar} (m-2-1);
    \end{tikzpicture}
