...
$S_1$ kommuniziert ebenfalls error-frei mit $U$. %TODO Satz umformulieren
...
\begin{proof}
  %TODO Beweis darauf anpassen, dass die Sprachen und Traces bewiesen werden
  %sollen und daraus dann die Implikation der Präkongruenzen folgern auch von
  %S/X auf S und Traces mit tau beachten
  Zuerst wird hier die Richtigkeit der Aussagen (i) bis (iii) gezeigt. Daraus
  kann dann der Rest des Satzes gefolgert werden.

  %TODO Umkerungen einarbeiten zu Trace aus S/X passenden in S finden, in dem
  %tau-Transitionen durch Outputs aus X ersetzt werden
  (i)
  ...
  \overset{a_n}{\Rightarrow}$. Hier ist wichtig zu beachten, dass die
  jeweiligen Zustände nicht exakt für eine Transition erreicht werden müssen.
  Es kann sich hier um eine Transitionsfolge aus beliebig vielen $\tau$s und
  dem jeweiligen $a_i\in\Sigma$ handeln. Dabei ist egal, an welcher Stelle das
  $a_i$ auftaucht. Dies ist notwendig, das auf Trace-Ebene nicht mehr
  ...
  \begin{itemize}
    \item Fall 1 ($n=0$): Es gilt $w=\varepsilon$. Somit enthält $w$ keine
      ...
      gilt also $w\in L(S/X)$. Somit ist für diesen Fall die Aussage über $L$
      korrekt.
    \item Fall 2 ($n\geq 1$): Nach der Internalisierung bleiben von dem Ablauf
      nur noch die Aktionen übrig, die nicht Elemente aus $X\cap O$ sind. Der
      Ablauf reduziert sich also auf $q_0 \overset{\tau~\mathrm{falls}~a_1\in
      X\cap O}{\overset{\mathrm{sonst}~a_1}{\Longrightarrow}} q_1
      \overset{\tau~\mathrm{falls}~a_2\in X\cap
      O}{\overset{\mathrm{sonst}~a_2}{\Longrightarrow}} \dots
      \overset{\tau~\mathrm{falls}~a_n\in X\cap
      O}{\overset{\mathrm{sonst}~a_n}{\Longrightarrow}} q_n$. Dabei bleibt
      durch das Hiding von Aktionen aus $X$ in $w$ nur noch
      $w|_{\Sigma\backslash (X\cap O)}$ erhalten. Diese Projektion des Wortes
      $w$ auf die eingeschränkte Aktionenmenge ist dann in $L(S/X)$ enthalten,
      da immer noch der selbe Zustand durch das Worte erreicht wird. Es gilt
      also auch für diesen Fall die Aussage über die Sprache $L$.
  \end{itemize}
  Für ein Wort aus der Sprache $L$ des Transitionssystems $S/X$ existiert wie
  oben auch ein Ablauf. Hier ist es jedoch wichtig, dass auch
  $\tau$-Transitionen gemacht werden um dieses Wort auszuführen. In dem dies
  $\tau$-Transitionen durch Transitionen mit Elementen aus $X\cap O$ ersetzt
  werden erhält man eine Transition aus der Sprache $L(S)$.

  (ii)
  Es wird einen Trace $w=a_1a_2\dots a_n\in \ET{}(S)$ gewählt. Dieser Traces
  muss nicht wie bei Punkt (i) einem Ablauf in $S$ entsprechen. Jedoch kann ein
  Präfix von $w$ gefunden werden, dass besondere Eigenschaften erfüllen soll
  und für dass es einen Ablauf gibt. Hierfür muss jedoch unterschieden werden,
  aus welchem Grund das $w$ in $\ET{}(S)$ enthalten ist.
  \begin{itemize}
    \item Fall 1 ($w\in\cont{}\PrET{}(S)$): In diesem Fall gibt es einen Ablauf
      für ein Präfix dieses $w$s, der zu einem Ablauf zu einem Zustand aus $E$
      ergänzt werden kann. Der Ablauf hat also die Form $q_0
      \overset{a_1}{\Rightarrow} q_1 \overset{a_2}{\Rightarrow} \dots
      \overset{a_m}{\Rightarrow} q_m \overset{b_1}{\Rightarrow} q_{m+1}
      \overset{b_2}{\Rightarrow} \dots \overset{b_l}{\Rightarrow} q_{m+l}$ mit
      $m\leq n$, $l\geq 0$ und $b_i\in O(S)\forall i$. Es gilt dann $a_1a_2\dots
      a_mb_1b_2\dots b_l\in \StET{}(S)$. Analog wie im Beweisteil zu (i) wird
      dieser Ablauf durch die Internalisierung reduziert. Somit ist die
      Projektion von $a_1a_2\dots a_mb_1b_2\dots b_l$ auf die eingeschränkte
      Aktionenmenge auf jeden Fall in $\StET{}(S/X)\subseteq\ET{}(S/X)$
      enthalten. Da $b_1b_2\dots b_l\in O(S)^*$ gilt, gilt nach der Projektion
      auch $(b_1b_2\dots b_l)|_{\Sigma\backslash (X\cap O)}\in O(S/X)^*$. Somit
      gilt $\prune{}((a_1a_2\dots a_m)|_{\Sigma\backslash (X\cap
      O)})=\prune{}((a_1a_2\dots a_mb_1b_2\dots b_l)|_{\Sigma\backslash (X\cap
      O)})$. Da \ET{} eine Menge ist, die nach Definition immer unter \cont{}
      abgeschlossen ist, sind alle Verlängerung von $(a_1a_2\dots
      a_m)|_{\Sigma\backslash (X\cap O)}$ ebenfalls in $\ET{}(S/X)$ enthalten.
      Es gilt also speziell auch $w|_{\Sigma\backslash (X\cap O)}\in
      \ET{}(S/X)$. Da alle Elemente aus $\ET{}(S/X)$ nur Aktionen aus
      $\Sigma\backslash (X\cap O)$ enthalten, ist ausgeschlossen, dass das eine
      Fortsetzung mit Aktionen außerhalb dieser Menge möglich ist.
    \item Fall 2 ($w\in\cont{}\MIT{}(S)$): In diesem Fall ist bereits für ein
      Präfix von $w$ ein Ablauf zu einem Zustand möglich, der nicht für alle
      Inputs eine Transitionsmöglichkeit bietet. Der Ablauf hat also die Form
      $q_0 \overset{a_1}{\Rightarrow} q_1 \overset{a_2}{\Rightarrow} \dots
      \overset{a_m}{\Rightarrow} q_m$ mit $m\leq n$ und $a_1a_2\dots a_m\in
      \MIT{}(S)$. Ab hier Verläuft die Argumentation für diesen Fall analog zu
      der vom letzten Fall, nur das die \prune{}-Funktion nicht verwendet
      werden muss, da es hier keine Verlängerungen um Outputs gab.
  \end{itemize}
  Ein Trace $w\in \ET{}(S/X)$ muss wie oben nicht im Transitionssystem
  enthalten sein. Es kann jedoch ein Präfix von $w$ gefunden werden wie oben,
  ...
  verlängert werden, so dass ein $w'\in \ET{}(S)$ entsteht, dass sich von $w$
  nur durch hinzugefügte Aktionen aus $X$ unterscheidet.

  (iii)
  Für einen Trace $w=a_1a_2\dots a_n\in \EL{}(S)$ gilt $w\in L(S)$ oder $w\in
  \ET{}(S)$. Für beide Fälle wurde oben bereits gezeigt, dass dann
  $w|_{\Sigma\backslash (X\cap O)}$ in der entsprechenden Menge des
  Transitionssystems $S/X$ enthalten ist. Da \EL{} als Vereinigung aus den
  Mengen $L$ und \ET{} definiert ist, ist dadurch dann auch gezeigt, dass
  $w|_{\Sigma\backslash (X\cap O)}\in \EL{}(S/X)$ gilt.\\
  Ebenfalls analog zu den beiden vorangegangen Punkten kann auch argumentiert
  ...
  $\EL{}(S_1/X)\subseteq \EL{}(S_2/X)$ gilt.\\%TODO überlegen ob besser begründen
