...
\begin{proof}
    ...
    \item Fall I ($v_1\in MIT_1$ und $v_1\neq\varepsilon$): Es gibt $q_{01}
      ...
      \begin{itemize}
        ...
        \item Fall Ib) ($a\in I_2$): Es gilt $v_2'=v_2''a$ mit $q_{02}
          ...
          ebenfalls ein fehlender Input. Jedoch gilt für beide Möglichkeiten,
          dass $a\in Synch(S_1,S_2)$ liegt, da die Menge der synchronisierten
          Handlungen bezüglich~\cite{Vogler2014EIO} erweitert wurde, somit
          reicht schon, wenn für einen der beiden Automaten der Input nicht
          möglich ist. Da $q_1 \overset{a}{\not{\hspace{-0.1cm}\rightarrow}}$
          gilt, können wir schließen, dass $(q_1,q_2)
          \overset{a}{\not{\hspace{-0.1cm}\rightarrow}}$ folgt. Somit können
          wir hier $v:=v''a=v'\in MIT_{12}$ wählen.
        \item Fall Ic) ($a\in O_2$): Es gilt $v_2'=v_2''a$ und $q_{02}
          \overset{v_2'}{\Rightarrow}$. Wir erhalten also $q_{02}
          ...
          E_{12}$. Wir wählen $v:=prune(v'')\in PrET_{12}$.
      \end{itemize}
  \item Fall II ($v_1\in PrET_1$): $\exists u_1\in O_1^*:q_{01}
    ...
    sein. Im Fall von $v_2\in MIT_2$ kann somit $a=b$ gelten und somit wäre
    $v_2=v_2'$. Dieser Fall verläuft jedoch analog zu Fall I und wird somit
