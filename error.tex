...
\begin{proof}
  ...
  zulässig sind. Um eine genauere Vorstellung davon zu bekommen, was
  das Hiding für die Systeme bedeutet, werden hier zunächst einmal die
  neuen Errortraces und die neue error-geflutete Sprache definiert. Es gilt:
  \begin{itemize}
    \item $\StET{}(S/X) = \{w\in (\Sigma\backslash (X\cap O))^*\mid \exists
      w'\in \StET{}(S): w'|_{\Sigma\backslash (X\cap O)} = w\}$,
    \item $\PrET{}(S/X) = \{\prune (w)\mid w\in\StET{}(S/X)\}$,
    \item $\MIT{}(S/X) = \{w\in (\Sigma\backslash (X\cap O))^*\mid \exists
      w'\in \MIT{}(S): w'|_{\Sigma\backslash (X\cap O)} = w\}$,
    \item $\ET{}(S/X) = \cont{}(\PrET{}(S/X))\cup \cont{}(\MIT{}(S/X))$,
    \item $L(S/X) = \{w\in (\Sigma\backslash (X\cap O))^*\mid \exists w'\in
      L(S): w'|_{\Sigma\backslash (X\cap O)} = w\}$,
    \item $\EL{}(S/X) = L(S/X)\cup \ET{}(S/X)$.
  \end{itemize}
  ...
  muss unterschieden werden, ob es sich hier um einen Traces handelt, der aus
  dem leeren Wort besteht oder um einen Traces, für den $m\geq 1$ gilt.
  \begin{itemize}
    ...
    \item Fall 2 ($m\geq 1$): Nach der Internalisierung bleiben von dem Ablauf
      ...
      $a_m\in I$ gelten. Der Trace reduziert sich also auf
  ...
  wichtig festzuhalten, dass für einen Trace aus $L_1$ keine beliebigen
