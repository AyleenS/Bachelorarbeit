...
\begin{proof}
  ...
  $\tau$s durch Outputs nicht an der Erreichbarkeit oder der Menge der Errors
  geändert werden. Es ist also jeder Error, der in $S_i/X$ lokal erreichbar ist
  auch in $S_i$ erreichbar. Somit folgt die Behauptung.
\end{proof}
...
\begin{proof}
  ...
  \begin{itemize}
    ...
    \item Fall 2 ($n\geq 1$): Nach der Internalisierung bleiben von dem Ablauf
      ...
      $w|_{\Sigma\backslash X}$ erhalten. Diese Projektion des Wortes
  ...
  ersetzt werden erhält man ein Trace aus der Sprache $L(S)$.
  ...
  \begin{itemize}
    \item Fall 1 ($w\in\cont{}(\PrET{}(S))$): In diesem Fall gibt es einen Ablauf
      ...
      $m\leq n$, $l\geq 0$ und $\forall i\in \{1,\dots ,l\}: b_i\in O(S)$. Es gilt dann $a_1a_2\dots
      ...
      auch $(b_1b_2\dots b_l)|_{\Sigma\backslash X}\in O(S/X)^*$ und
      somit $\prune{}((a_1a_2\dots a_m)|_{\Sigma\backslash
      ...
      Es gilt also speziell auch $w|_{\Sigma\backslash X}\in
      \ET{}(S/X)$. Da alle Elemente aus $\ET{}(S/X)$ nur Aktionen aus
      $\Sigma\backslash X$ enthalten, ist ausgeschlossen, dass das eine
    ...
    \item Fall 2 ($w\in\cont{}(\MIT{}(S))$): In diesem Fall ist bereits für ein
      ...
      $a_1a_2\dots a_m\in \MIT{}(S)$. Ab hier Verläuft die Argumentation für
      diesen Fall analog zu der vom letzten Fall, nur das die \prune{}-Funktion
      nicht verwendet werden muss, da es hier keine Verlängerungen um Outputs
      gab.
  \end{itemize}
  ...
  $w|_{\Sigma\backslash X}$ in der entsprechenden Menge des
  ...
  $w|_{\Sigma\backslash X}\in \EL{}(S/X)$ gilt.\\
