...
\begin{proof}
  ...
  \begin{itemize}
    \item Fall 1a) ($a\in (I_1\cap I_2)$): Nun können wir den Ablauf der
      ...
      EL_1$ und $x_2\in cont(MIT_2)\subseteq ET_2\subseteq EL_2$ oder $x_2\in
      EL_2$ folgern. Damit folgt $w\in (x_1\|x_2)\cdot\{a\}\subseteq
      (x_1a)\|(x_2a)\subseteq ET_1\|EL_2$, und somit ist $w$ in der rechten
  ...
  \begin{itemize}
    \item Fall I ($v_1\in MIT_1$ und $v_1\neq\varepsilon$): Es gibt $q_{01}
      ...
      einen weiteren Fall einfügen, da es zulässsig ist, das $a$ sowohl ein in
      $I_1$, wie auch in $I_2$ enthalten ist.
      \begin{itemize}
        \item Fall Ia) ($a\notin\Sigma _2$): Durch (*) folgt $q_{02}
          \overset{v_2'}{\Rightarrow} q_2$ mit $v''\in v_1'\|v_2'$. Dadurch erhalten
        ...
        \item Fall Ib) ($a\in I_2$): Es gilt $v_2'=v_2''a$ mit $q_{02}
          \overset{v_2''}{\Rightarrow}$ und $v''\in v_1'\|v_2''$, wegen (*). In
          diesem Fall kann entweder gelten $q_2 \overset{a}{\rightarrow}$ oder $q_2
          \overset{a}{\not{\hspace{-0.1cm}\rightarrow}}$. Falls die zweite
          Möglichkeit gilt, ist $v_2'\in MIT_2$ und $a$ ist für $S_2$
          ebenfalls ein fehlender Input. Da jedoch für beide Möglichkeiten
          gilt, dass $a\in Synch(S_1,S_2)$ liegt, da die Menge der
          synchonisierten Handlungen bezüglich~\cite{Vogler2014EIO} erweiter
          wurde, reicht schon, wenn für einen der beiden Automaten der Input
          nicht möglich ist. Da also $q_1
          \overset{a}{\not{\hspace{-0.1cm}\rightarrow}}$ gilt, können wir
          schließen, dass $(q_1,q_2)
          \overset{a}{\not{\hspace{-0.1cm}\rightarrow}}$ gilt. Wir könnnen also
          hier wählen $v:=v''a=v'\in MIT_{12}$.
        \item Fall Ic) ($a\in O_2$): Es gilt $v_2'=v_2''a$. Durch (*) erhalten wir $q_{02}
      ...
      \end{itemize}
  \item Fall II ($v_1\in PrET_1$): $\exists u_1\in O_1^*:q_{01}
...
sollen. Dadurch ändern sich natürlich auch Teile des Beweises.
