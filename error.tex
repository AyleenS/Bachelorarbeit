  ...
  \begin{center}
    \begin{tikzpicture}
      \matrix (m) [matrix of math nodes,row sep=3em,column sep=4em,minimum width=2em]{
        ...
        \forall~\mathrm{Partner}~U:U\|S_1\EBRel U\|S_2 &
        \forall~\mathrm{komponierbaren}~U:U\|S_1\EBRel U\|S_2 \\};
        \path[-stealth]
          (m-1-1) edge [double,semithick] node [above] {\glqq{}$\Leftarrow$\grqq{} von
            Satz~\ref{satzFullAbstractness}} (m-1-2)
          (m-1-2) edge [double,semithick] node [right] {\glqq{}$\Rightarrow$\grqq{} von
            Satz~\ref{satzFullAbstractness}} (m-2-2)
          (m-2-1) edge [double,semithick] node [left] {Lemma~\ref{lemVerfeinerung}} (m-1-1)
          (m-2-2) edge [double,semithick] node [below] {$U$ Partner
          $\Rightarrow U$ komponierbar} (m-2-1);
    \end{tikzpicture}
