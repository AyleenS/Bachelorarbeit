...
Die Verfeinerungsrelation, in dem nächsten Lemma betrachtet werden soll, soll
über gute Kommunikation im Sinne der error-freien Kommunikation verfeinern.

\begin{lem}[Verfeinerung mit Errors]
  \label{lemVerfeinerung}
  Gegeben sind zwei \EIO{}s $S_1$ und $S_2$ mit der gleichen Signatur. Wenn
  $U\|S_1 \EBRel U\|S_2$ für alle Partner $U$ gilt, dann folgt daraus $S_1\ERel
  S_2$.
\end{lem}
...
\begin{proof}
  ...
  ohne Hiding ein passender in dem System ohne Hiding gefunden werden kann,
  ...
  Transitionsmöglichkeit. Es gilt also entweder $w\in \PrET{}_1$ oder $w\in
  \MIT{}_1$. An diesem Punkt muss unterschieden werden, ob es sich hier um ein
  Trace handelt, der aus dem leeren Wort besteht oder um ein Trace, für den
  $m\geq 1$ gilt.
  \begin{itemize}
    ...
    \item Fall 2 ($m\geq 1$): Nach der Internalisierung bleiben von dem Ablauf
      ...
      \overset{a_m}{\rightarrow} q_m$. Dabei bleibt durch das Hiding von dem
      $w$ nur noch $w|_{\Sigma\backslash (X\cap O)}$ erhalten. Diese
      Projektion des Wortes $w$ auf die eingeschränkte Aktionsmenge ist dann
      in $\ET{}(S_1/X)$ enthalten, da immer noch der selbe Zustand erreicht
      wird, der bereits in $\ET{}_1$ als Fehler-Zustand erkannt wurde. Es sind
      auch alle beliebigen Verlängerungen analog zum letzten Fall in den beiden
      Mengen der Errortraces enthalten.
  \end{itemize}
  Nach Voraussetzung gilt $w\in\ET{}_2$, wenn $w\in\ET{}_1$. Nach analoger
  Argumentation gilt dann auch $w|_{\Sigma\backslash (X\cap
  O)}\in\ET{}(S_2/X)$.\\
