...
\begin{proof}
  ...
  \overset{a_n}{\Rightarrow} q_n$ mit $q_n=q$. Hier ist wichtig zu beachten, dass die
  jeweiligen Zustände nicht exakt über eine Transition erreicht werden müssen.
  ...
  $a_i$ auftaucht. Dies ist notwendig, da auf Trace-Ebene nicht mehr
  ...
  werden erhält man ein Trace aus der Sprache $L(S)$.
  ...
  \begin{itemize}
    \item Fall 1 ($w\in\cont{}(\PrET{}(S))$): In diesem Fall gibt es einen Ablauf
      ...
      $m\leq n$, $l\geq 0$ und $\forall i\; b_i\in O(S)$. Es gilt dann $a_1a_2\dots
      ...
      enthalten. Da $b_1b_2\dots b_l\in O(S)^*$, gilt nach der Projektion
      auch $(b_1b_2\dots b_l)|_{\Sigma\backslash (X\cap O)}\in O(S/X)^*$ und
      somit $\prune{}((a_1a_2\dots a_m)|_{\Sigma\backslash (X\cap
      O)})=\prune{}((a_1a_2\dots a_mb_1b_2\dots b_l)|_{\Sigma\backslash (X\cap
      O)})$. Da \ET{} eine Menge ist, die nach Definition unter \cont{}
  ...
  bzw.\ $\EL{}(S)$ ein entsprechender Trace aus $\ET(S/X)$ bzw.\ $\EL{}(S/X)$
