...
z.B.\ wird $I_1$ für die Inputmenge des Transitionssystem $S_1$ geschrieben. Diese
...
muss, werden diese gemeinsamen Inputs synchronisiert.
...
Zustände erreicht. Um dies besser umsetzten zu können, wird eine
...
beiden Systeme und die Systeme selbst komponierbar sind. Dadurch, dass $w$ so
gewählt wurde, dass die Projektion auf die einzelnen Alphabete die jeweiligen
Wörter ergibt, kann ohne interne Aktionen gesagt werden, dass nur ein Ablauf
möglich ist. Da jedoch auch interne Aktionen zulässig sind, sind
mehrere Abläufe möglich, da nicht klar ist, wann ein $\tau$ in dem Trace
ausgeführt wird. Daraus ergibt sich das folgende Lemma.
...
Hiding wurde bereits in~\cite{Chilton2013} auf Traces betrachtet. Da hier
die Betrachtungsweise von Transitionssystemen aus startet, wird auch Hiding aus
der Sicht dieser Systeme definieren, wie
in~\cite{Schlosser2012BA}. Eine ähnliche Betrachtung für Hiding bei \EIO{}s
wurde auch bereits in~\cite{Lynch1996} umgesetzt. Dort werden nur
Output-Aktionen internalisiert, jedoch gibt es eine Menge an internen Aktionen
und
...
Outputs sind diese nicht mehr nach außen sichtbar. Werden jedoch Inputs
...
in der folgenden Definition nur die Internalisierung von Outputs erlaubt,
...
\begin{Def}[Internalisierungsoperator]
  ...
  \begin{itemize}
    \item $\forall x\in X: x\in O\wedge x\neq\tau$.
...
synchronisierten Aktionen betrachtet. Diese Parallelkomposition kann nun
mit dem Internalisierungsoperator durch Hiding der synchronisierten Aktionen
nachbildet werden. Da diese synchronisierten Aktionen in der Parallelkomposition dieser
Arbeit als Outputs des Gesamtsystems übernommen werden, können dadurch auch
wirklich alle synchronisierten Aktionen verborgen werden.
