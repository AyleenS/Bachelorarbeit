...
Die nicht mit den entsprechenden Outputs synchronisierten Inputs der zu
komponierenden \EIO{}s werden als Inputs der Parallelkomposition übernommen.
%TODO noch mal über den Satz nachdenken
...
Outputs, die in einer Komposition synchronisiert werden. Hier werden diese
Aktionen hingegen nicht verborgen. Jedoch wird im weiteren Verlauf noch
...
unsynchronisierbaren Outputs besitzen (neue Errors) zusammen.\\
...
muss, werden die gemeinsamen Inputs synchronisiert.
...
\end{aligned}\hspace{1cm}\right\}
  ...
  $Sig(S_1)=(I,O)$ und $Sig(S_2)=(O,I)$ haben. %TODO Trace=Wort?
\end{Def}
...
$\prune{}$-Funktion definiert, die alle Outputs am Ende eines Traces entfernt. Zusätzlich
...
in dem Trace ausgeführt wird. Daraus ergibt sich das folgende Lemma.
...
Internalisierungsoperator durch Hiding der synchronisierten Aktionen nachbildet
werden, die in der Parallelkomposition zu Outputs werden. Da in dieser Arbeit
...
jedoch mit unserer Definition des Internalisierungsoperators nicht verborgen
werden. Dies wäre auch nicht sinnvoll, da diese Synchronisation von Inputs
keine Kommunikation zwischen den Systemen war, sondern nur eine Zusammenfügung,
...
\begin{Def}[Parallelkomposition mit Internalisierung]
  ...
  $S_1|S_2=(S_1\|S_2)/(Synch(S_1,S_2)\cap O_{12})$.
\end{Def}
