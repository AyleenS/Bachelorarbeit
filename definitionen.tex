...
Zusätzlich werden Funktionen definiert, die die Traces beliebig fortsetzen. Da
nicht nur beliebige Fortsetzungen von gekürzten Traces betrachtet werden
sollen, wird ebenfalls noch eine andere Funktion zum kürzen und verlängern
eingeführt, die nur Verlängerungenen von Traces zulässt, die bereits im
eingegebenen Trace möglich waren.
...
\begin{Def}[Pruning- und Fortsetzungs-Funktion]
  ...
  \begin{itemize}
    ...
    \item $\prunenew{}:\Sigma ^*\rightarrow\mathfrak{P}(\Sigma ^*), w\mapsto
      \{u\mid \exists v\in O ^*: uv=w\}$,
    \item $\prunenew{}:\mathfrak{P}(\Sigma ^*)\rightarrow\mathfrak{P}(\Sigma
      ^*), L\mapsto\bigcup\{\prunenew{}(w)\mid w\in L\}$,
...
von Restriktionen in CSS, siehe dazu~\cite{Milner1989} ,verboten. Diese Art der
